...
Команда `/admin_import` принимает обычный XLSX или SQLite bundle формата `golden-vpn.typed-keys.v1`. Для каждой строки сохраняются тип `awg`, `trojan`, `hysteria`, `vless` или `ikev2` и статус `available` либо `issued`. Перед подтверждением бот показывает количество строк по типам и статусам; дубликаты и неизвестные тарифы отклоняются.
